...
// Adapter handles MySQL database operations
type Adapter struct {
	logger *zap.Logger
}
...
func (a *Adapter) Execute(ctx context.Context, task *executor.Task) (*executor.TaskResult, error) {
	...
	a.logger.Info("Executing MySQL task",
...
// TaskConfig holds the configuration for a MySQL task
...
func parseConfig(raw map[string]interface{}) (*TaskConfig, error) {
	...
	if config.Connection.TLSMode == "" {
		config.Connection.TLSMode = "false"
	}
